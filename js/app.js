  ...
  if (state.renderer.xr.isPresenting) {
    ...
    if (session) {
      ...
      session.inputSources.forEach((source, index) => {
        // Read B/Y button (button index 5 on Quest/Oculus controllers)
    ...
    // Update popped model rotation and scale if active in VR
    if (state.poppedModel) {
      // Auto-rotation disabled when model is gripped (kept stationary)
      // state.poppedModel.rotation.y += 0.01;
      if (state.grippedControllers.length === 2) {
        const currentDistance = getGripMidpointDistanceToCamera();
        const initialDistance = state.initialZoomDistance;
        if (initialDistance > 0 && currentDistance > 0) {
          const delta = initialDistance - currentDistance;
          const baseScale = state.initialPoppedScale;
          let newScale = baseScale * (1 + delta * 2.0);
          const minScale = state.poppedModel.userData.baseScale * 0.2;
          const maxScale = state.poppedModel.userData.baseScale * 5.0;
          newScale = Math.max(minScale, Math.min(maxScale, newScale));
          state.poppedModel.scale.setScalar(newScale);
        }
      }
    }
